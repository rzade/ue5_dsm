...
bool UBlueprintJsonObject::ParseToMap(FString JsonString, TArray<UBlueprintJsonObject*>& JsonArrayObject) {
	...
	for (auto JsonObject : JsonArray) {
		...
		RequestObj->SetObjectField("item", JsonObject->AsObject());

		FString JsonObjectString;
		TSharedRef<TJsonWriter<TCHAR, TCondensedJsonPrintPolicy<TCHAR>>> Writer = TJsonWriterFactory<TCHAR, TCondensedJsonPrintPolicy<TCHAR>>::Create(&JsonObjectString);
		FJsonSerializer::Serialize(RequestObj, Writer);

		UBlueprintJsonObject* JsonObjectBjo = UBlueprintJsonObject::Create();
		UBlueprintJsonObject::Parse(JsonObjectString, JsonObjectBjo);
		JsonArrayObject.Add(JsonObjectBjo);
	}
